    ...
    for (i = 0; i < s->nb_streams; i++) {
        ...
        if (os->segment_type == SEGMENT_TYPE_MP4) {
            if (c->streaming)
                ...
                if (c->global_sidx) {
                    // upstream: av_dict_set(&opts, "movflags", "+dash+delay_moov+global_sidx+skip_trailer", AV_DICT_APPEND);
                    ...
                    // upstream: av_dict_set(&opts, "movflags", "+dash+delay_moov+skip_trailer", AV_DICT_APPEND);
                    if (c->start_segment > 1) {
